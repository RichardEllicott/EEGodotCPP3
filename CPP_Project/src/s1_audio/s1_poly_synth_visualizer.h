...
#ifndef S1_POLY_SYNTH_VISUALIZER_H
#define S1_POLY_SYNTH_VISUALIZER_H
...
class S1PolySynthVisualizer : public Control {
    GDCLASS(S1PolySynthVisualizer, Control)
    ...
    DECLARE_PROPERTY_SINGLE_FILE_DEFAULT(int, draw_wave_mode, 0)
   ...
   public:
    Ref<Image> image;  // Ref to store the image (TESTING)
   ...
   protected:
    static void _bind_methods() {
        ...
        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, BOOL, enabled);       // just crashes with signal or not
        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, VECTOR2I, grid_size)  // note Variant::FLOAT is also valid
        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, VECTOR2, cell_size)   // note Variant::FLOAT is also valid
        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, FLOAT, cell_border)   // note Variant::FLOAT is also valid

        CREATE_CLASS_BINDINGS(S1PolySynthVisualizer, "ImageTexture", image_texture)  // maybe the texture causes crashing?????? (i have ide crashes)

        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, NODE_PATH, poly_synth)

        // wave draw
        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, BOOL, draw_wave)
        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, INT, draw_wave_frames)
        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, INT, draw_wave_mode)
        CREATE_VAR_BINDINGS(S1PolySynthVisualizer, VECTOR2, draw_wave_size)
        // CREATE_VAR_BINDINGS(S1PolySynthVisualizer, VECTOR2, draw_wave_size)
        ...
        ClassDB::bind_method(D_METHOD("macro_test"), &S1PolySynthVisualizer::macro_test);
        ClassDB::bind_method(D_METHOD("macro_test2", "input", "number"), &S1PolySynthVisualizer::macro_test2);  // method bind with pars
    }
    ...
    void _draw_waveform() {
        ...
        if (_poly_synth) {
            ...
            for (int x = 0; x < draw_wave_frames; x++) {
                ...
                Vector2 pos2 = Vector2(x, val2.x);
                pos2 *= draw_wave_size;

                switch (draw_wave_mode) {
                    case 0:
                        rs->canvas_item_add_circle(canvas_rid, pos1, 1.0, Color(1, 1, 1), false);
                        break;
                    case 1:

                        rs->canvas_item_add_line(canvas_rid, pos1, pos2, Color(1, 1, 1), 1.0);
                        break;
